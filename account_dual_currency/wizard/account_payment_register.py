...
class AccountPaymentRegister(models.TransientModel):
    ...
    payment_difference_bs = fields.Monetary(string="Diferencia Bs", currency_field='source_currency_id', digits='Dual_Currency')
    ...
    def _compute_payment_difference(self):
        for wizard in self:
            ...
            if wizard.source_currency_id == wizard.currency_id:
                ...
                wizard.payment_difference_bs = wizard.source_amount - wizard.amount
            elif wizard.currency_id == wizard.company_id.currency_id:
                # Payment expressed on the company's currency.
    ...
    def _get_wizard_values_from_batch(self, batch_result):
        ...
        if not self.tax_today:
            tax_today = lines[0].company_id.currency_id_dif.tasa_referencia
        else:
    ...
    def _create_payments(self):
        self.env.context = dict(self.env.context, tasa_factura=self.tax_today)
        ...
        if self.source_amount == 0:
            ...
            if not self.currency_id.is_zero(self.payment_difference) and self.payment_difference_bs < 0 and self.payment_difference_handling == 'reconcile':
                ...
                if self.payment_difference_bs > 0:
                    ...
                    move['line_ids'][0][2]['partner_id'] = self.partner_id.id if payments.payment_type == 'inbound' else False
