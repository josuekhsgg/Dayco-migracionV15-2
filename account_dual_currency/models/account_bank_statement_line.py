...
class AccountBankStatementLine(models.Model):
    ...
    amount_usd_statement = fields.Monetary(currency_field='currency_id_dif_statement', string='Total Ref.', store=True, readonly=True, compute='_amount_usd',
    ...
    def _get_default_tasa(self):
        for rec in self:
            ...
            if rec.date:
                domain = [('name','=',rec.date),('currency_id','=',rec.currency_id_dif_statement.id),('company_id','=',self.env.company.id)]
                #print(domain)
                tasa_fecha = self.env['res.currency.rate'].search(domain)
                if tasa_fecha:
                    tasa_referencia_statement = tasa_fecha.tasa_referencia
                else:
                    tasa_referencia_statement = self.env.company.currency_id_dif.tasa_referencia
            else:
                tasa_referencia_statement = self.env.company.currency_id_dif.tasa_referencia
            print(tasa_referencia_statement)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if 'tasa_referencia_statement' not in vals:
                vals['tasa_referencia_statement'] = self._get_default_tasa()
    ...
    def _prepare_liquidity_move_line_vals(self):
        ...
        if balance_usd == 0:
            tasa = (self.tasa_referencia_statement if self.tasa_referencia_statement else self._get_default_tasa())
            balance_usd = balance / tasa if tasa > 0 else 1
    ...
    def _prepare_counterpart_move_line_vals(self, counterpart_vals, move_line=None):
        ...
        if company_currency != journal_currency:
            ...
            amount_residual_usd = -counterpart_vals.pop('amount_residual_usd', move_line.amount_residual_usd if move_line else 0.0)
