...
class Productos(models.Model):
    ...
    standard_price_usd = fields.Float(string="Costo $", inverse='_set_standard_price_usd', compute='_compute_standard_price_usd', readonly=False, store=True, digits='Dual_Currency')
    costo_reposicion_usd = fields.Float(string="Costo Reposición $", digits='Dual_Currency')
        ...
        for rec in self:
            if rec.list_price_usd:
                if rec.list_price_usd >0:
                    ...
                    if tasa:
                        rec.list_price = rec.list_price_usd * tasa.tasa_referencia
