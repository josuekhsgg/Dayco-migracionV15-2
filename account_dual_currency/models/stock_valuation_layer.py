            ...
            if not 'quantity' in vals and self.stock_move_id:
                ...
                if 'unit_cost' in vals:
                    ...
                    if vals.get('stock_move_id') or self.stock_move_id:
                        stock_move_id = self.env['stock.move'].search([('id', '=', vals.get('stock_move_id') or self.stock_move_id.id)])
                        if stock_move_id.location_id.usage == 'supplier':
                            self.product_id.standard_price_usd = standard_price_usd
                if 'value' in vals:
        ...
        if 'account_move_id' in vals:
            ...
            for rec in self:
                if rec.stock_move_id:
                    ...
                    if new_rate_ids:
                        new_rate = 1 / new_rate_ids[self.env.company.currency_id_dif.id]
                move_id = self.env['account.move'].sudo().with_context(check_move_validity=False).search([('id', '=', vals['account_move_id'])])
                if move_id:
                    ...
                    if val.get('quantity'):
                        if float(val['quantity']) != 0:
                            ...
                            if 'stock_move_id' in val:
                                ...
                                standard_price_usd = product_id.standard_price_usd
                            val['unit_cost_usd'] = standard_price_usd
                            val['value_usd'] = float(val['quantity']) * standard_price_usd
                            val['tasa'] = new_rate
                            if product_id.cost_method in ('average', 'fifo') and supplier:
                            ...
                            if val.get('stock_move_id'):
                                ...
                                if stock_move_id.location_id.usage == 'supplier':
                                    product_id.standard_price_usd = standard_price_usd
                                    #product_id.standard_price = standard_price_usd * tasa.tasa_referencia
                        else:
                            product_id = self.env['product.product'].search([('id', '=', val['product_id'])])
                    ...
                    if val.get('stock_move_id'):
                        ...
                        if stock_move_id.location_id.usage == 'supplier':
                            if product_id.cost_method == 'average':
                                if product_id.standard_price_usd == 0:
                                    product_id.standard_price_usd = val['unit_cost_usd']
        else:
            if not 'unit_cost_usd' in vals and not 'value_usd' in vals:
                if 'quantity' in vals:
                ...
                if vals.get('stock_move_id'):
                    ...
                    if stock_move_id.location_id.usage == 'supplier':
                        if product_id.cost_method == 'average':
                            if product_id.standard_price_usd == 0:
                                product_id.standard_price_usd = vals['unit_cost_usd']
        ...
        for sl in res:
            if sl.stock_move_id:
                if sl.stock_move_id.location_id.usage == 'supplier' and not sl.stock_landed_cost_id:
                    if sl.product_id.cost_method == 'average':
                        if sl.product_id.standard_price_usd > 0:
                            sl.product_id.standard_price_usd = sl.product_id.value_usd_svl / sl.product_id.quantity_svl
        return res