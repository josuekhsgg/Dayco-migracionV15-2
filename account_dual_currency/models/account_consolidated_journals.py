...
class report_account_consolidated_journal(models.AbstractModel):
    ...
    def _get_sum(self, results, lambda_filter, options):
        ...
        sum_debit = self.format_value(sum([r['debit'] for r in results if lambda_filter(r)]))  if currency_dif == 'Bs' else self.format_value_usd(sum([r['debit'] for r in results if lambda_filter(r)]))
        sum_credit = self.format_value(sum([r['credit'] for r in results if lambda_filter(r)])) if currency_dif == 'Bs' else self.format_value_usd(sum([r['credit'] for r in results if lambda_filter(r)]))
        sum_balance = self.format_value(sum([r['balance'] for r in results if lambda_filter(r)])) if currency_dif == 'Bs' else self.format_value_usd(sum([r['balance'] for r in results if lambda_filter(r)]))
    ...
    def _get_journal_line(self, options, current_journal, results, record):
        return {
                ...
                'level': 2,
                'columns': [{'name': n} for n in self._get_sum(results, lambda x: x['journal_id'] == current_journal, options)],
                'unfoldable': True,
    ...
    def _get_account_line(self, options, current_journal, current_account, results, record):
        return {
                ...
                'level': 3,
                'columns': [{'name': n} for n in self._get_sum(results, lambda x: x['account_id'] == current_account and x['journal_id'] == current_journal,options)],
                'unfoldable': True,
    ...
    def _get_line_total_per_month(self, options, current_company, results):
        ...
        lines.append({
                    ...
                    'class': 'total',
                    'level': 1,
                    'columns': [{'name': n} for n in self._get_sum(results, lambda x: x['company_id'] == current_company,options)]
        })
        lines.append({
        ...
        if dates:
            ...
            for date in sorted(dates):
                ...
                sum_debit = self.format_value(sum([r['debit'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company])) if currency_dif == 'Bs' else self.format_value_usd(sum([r['debit'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company]))
                sum_credit = self.format_value(sum([r['credit'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company])) if currency_dif == 'Bs' else self.format_value_usd(sum([r['credit'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company]))
                sum_balance = self.format_value(sum([r['balance'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company])) if currency_dif == 'Bs' else self.format_value_usd(sum([r['balance'] for r in results if (r['month'] == month and r['yyyy'] == year) and r['company_id'] == current_company]))
                vals = {
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if currency_dif == 'Bs':
            ...
            select = """
                    SELECT to_char("account_move_line".date, 'MM') as month,
                           to_char("account_move_line".date, 'YYYY') as yyyy,
                           COALESCE(SUM("account_move_line".debit_usd - "account_move_line".credit_usd), 0) as balance,
            ...
            if self._need_to_unfold('account_%s_%s' % (values['account_id'], values['journal_id']), options):
                vals = {
                    ...
                    'columns': [{'name': n} for n in
                                [self.format_value(values['debit']) if currency_dif == 'Bs' else self.format_value_usd(values['debit']), self.format_value(values['credit']) if currency_dif == 'Bs' else self.format_value_usd(values['credit']),
                                 self.format_value(values['balance']) if currency_dif == 'Bs' else self.format_value_usd(values['balance'])]],
                }
                lines.append(vals)
