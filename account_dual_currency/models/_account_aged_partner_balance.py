...
class ReportAccountAgedPartnerUSD(models.AbstractModel):
    _name = "account.aged.partner_usd"
    ...
    def _get_templates(self):
        # OVERRIDE
        templates = super(ReportAccountAgedPartnerUSD, self)._get_templates()
    ...
    def _get_sql(self):
        ...
        a = self.env.cr.mogrify(query, params).decode(self.env.cr.connection.encoding)
        #print(query)
        return a
...
class ReportAccountAgedReceivable(models.Model):
    _name = "account.aged.receivable_usd"
    _description = "Vencida por Cobrar USD"
    _inherit = "account.aged.partner_usd"
    _auto = False
    ...
    def _get_report_name(self):
        return _("Vencida por Cobrar USD")
...
class ReportAccountAgedPayableUSD(models.Model):
    _name = "account.aged.payable_usd"
    _description = "Vencida por Pagar USD"
    _inherit = "account.aged.partner_usd"
    _auto = False
    ...
    def _get_options(self, previous_options=None):
        # OVERRIDE
        options = super(ReportAccountAgedPayableUSD, self)._get_options(previous_options=previous_options)
        options['filter_account_type'] = 'payable'
        return options

    @api.model
    def _get_report_name(self):
        return _("Vencida por Pagar USD")

    @api.model
    def _get_templates(self):
        # OVERRIDE
        templates = super(ReportAccountAgedPayableUSD, self)._get_templates()
