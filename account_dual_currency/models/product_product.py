...
class ProductProduct(models.Model):
    ...
    list_price_usd = fields.Float(string="Precio de venta $", digits='Dual_Currency')
    standard_price_usd = fields.Float(string="Costo $", digits='Dual_Currency', store=True)
    ...
    def _compute_value_usd_svl(self):
        ...
        for group in groups:
            ...
            product.value_usd_svl = self.env.company.currency_id_dif.round(group['value_usd'])
            products |= product
        ...
        for rec in self:
            if rec.standard_price_usd and rec.categ_id.property_valuation == 'manual_periodic':
                if rec.standard_price_usd > 0:
                    ...
                    if tasa:
                        rec.standard_price = rec.standard_price_usd * tasa.tasa_referencia
    ...
    def _prepare_in_svl_vals(self, quantity, unit_cost):
        ...
        unit_cost_usd = float(unit_cost) / tasa.tasa_referencia
        vals = {
            'product_id': self.id,
    ...
    def _run_fifo(self, quantity, company):
        ...
        if float_is_zero(qty_to_take_on_candidates, precision_rounding=self.uom_id.rounding):
            if tmp_value_usd != 0:
                tasa_tmp = round(tmp_value / tmp_value_usd, 4)
            else:
                tasa_tmp = 1
            vals = {
                'value': -tmp_value,
    ...
    def price_compute(self, price_type, uom=False, currency=False, company=None):
        ...
        for product in products:
            ...
            if price_type == 'standard_price':
                prices[product.id] = prices[product.id] * self.env.company.currency_id_dif.tasa_referencia
